<div>
    <button wire:click="openModal"
        class="bg-gradient-to-r from-green-500 to-green-300 h-10 transition duration-200 hover:from-transparent hover:to-transparent hover:bg-green-600 shadow-md font-bold text-white px-3 rounded-lg cursor-pointer max-[768px]:px-1">
        <span class="flex">
            Añadir
            <svg class="ml-1 " xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                fill="#FFFFFF">
                <path
    ...
    <div x-data="{ showModal: @entangle('showModal') }" x-show="showModal" x-transition:enter="ease-out duration-300"
        ...
        <div x-show="showModal" x-transition:enter="ease-out duration-300"
            ...
            class="relative bg-gradient-to-r from-green-600 to-green-800 rounded-xl max-w-3xl w-full m-auto px-1">

            <header class="">
                <div class="w-full flex justify-end pr-5 pt-5 pb-2">
                    <button wire:click="closeModal" title="Cerrar"
                ...
                <h2
                    class="text-center text-white bg-green-400/60 w-150 h-15 rounded-xl pt-2 mx-auto font-extrabold text-4xl my-2 max-[768px]:w-auto">
                    Nuevo Usuario</h2>
            ...
            <form wire:submit.prevent="save">
                        ...
                        <div class="flex flex-col-reverse">
                            <div class="flex">
                                ...
                                <input id="telefono-input" type="text" wire:model.defer="telefono"
                                    placeholder="XXX-XXXX" maxlength="8" pattern="\d{3}-\d{4}"
                    ...
                    <div class="my-5">
                        ...
                        <div class="flex flex-col-reverse">
                            <input id="direccion-input" type="text" wire:model.defer="direccion"
                                placeholder="Ingrese la dirección donde recide"
                                class="bg-gray-200 text-lg w-full p-1 pl-2 inset-shadow-sm rounded text-zinc-900 outline-none border-1 border-gray-600 focus:border-cyan-500 peer"
                                required>
                ...
                <footer class="my-3 mx-5 max-[768px]:my-6">
                    <div class="flex justify-end space-x-2 max-[768px]:justify-center">
                        <button type="submit" wire:loading.attr="disabled" wire:target="save" title="Añadir"
                            class="disabled:opacity-50 disabled:cursor-progress flex justify-center px-8 py-2 bg-gradient-to-r from-green-500 to-green-400 cursor-pointer text-white rounded shadow-lg shadow-black/50 transition duration-200 hover:from-transparent hover:to-transparent hover:bg-green-600 max-[768px]:px-15">
                            ...
                            <span wire:loading.flex wire:target="save" class="flex justify-center">
...
<script>
    document.addEventListener('livewire:init', () => {
        Livewire.hook('element.updated', () => {
            initToggles(), initInputs()
        });
        initInputs = () => {
            const e = document.getElementById('telefono');
            e && (e.replaceWith(e.cloneNode(!0)), document.getElementById('telefono')
                ?.addEventListener('input', e => {
                    let t = e.target.value.replace(/\D/g, '');
                    t.length > 3 && (t = t.substring(0, 3) + '-' + t.substring(3, 7)), e
                        .target.value = t
                }));
        };
        initToggles(), initInputs();
